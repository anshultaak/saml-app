...
class ServiceProvider(Document):
    ...
    provider_type = StringField(required=True)  # 'aws', 'jenkins', 'github', 'generic'
    ...
    custom_attributes = DictField()  # Provider-specific configuration
    attribute_mapping = DictField()  # Custom attribute mappings for SAML responses
    active = BooleanField(default=True)
    created_at = DateTimeField(default=datetime.utcnow)
