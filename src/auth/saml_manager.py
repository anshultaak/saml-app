...
class SAMLManager:
        ...
        try:
            ...
            try:
                if isinstance(client, str):
                    logging.error(f"SAML client initialization failed: {client}")
                    return redirect(url_for('auth.login'))
    ...
    def process_sso_request(self, sp_id=None, relay_state=None, saml_request=None, sig_alg=None, signature=None):
        ...
        try:
            ...
            if handler.get_provider_type() == 'github':
                ...
                saml_response = handler.build_saml_response(current_user, sp, in_response_to)
            else:
                logging.info(f"[SAML] Building AWS SAML response with InResponseTo: {in_response_to}")
                saml_response = handler.build_saml_response(current_user, sp, in_response_to)
    ...
    def build_github_response(user, sp, in_response_to=None):
        ...
        authn_statement.set("SessionIndex", f"id{int(time.time() * 1000)}.{random.randint(100000, 999999)}")
