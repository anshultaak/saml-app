"""
GitHub-specific SAML response builder.
Handles SAML authentication flows specifically for GitHub.
"""
import logging
import base64
...
class GitHubSAMLResponseBuilder:
    """Class for building SAML responses specifically for GitHub."""
    
    @staticmethod
    def build_github_response(user, sp, in_response_to=None):
        """
        Build a SAML response specifically formatted for GitHub.
        
        Args:
            user: The authenticated user
            sp: The service provider (GitHub)
        ...
        try:
            ...
            # Add name attribute
            name_attribute = SubElement(attribute_statement, QName(nsmap['saml'], 'Attribute'))
            name_attribute.set('Name', 'name')
            name_attribute.set('NameFormat', 'urn:oasis:names:tc:SAML:2.0:attrname-format:basic')
            name_value = SubElement(name_attribute, QName(nsmap['saml'], 'AttributeValue'))
            ...
            logging.error(f"Error building GitHub SAML response: {str(e)}")
            raise 