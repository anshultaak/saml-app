...
from ..auth.provider_registry import provider_registry
...
def saml_login(sp_id):
    ...
    provider_type = handler.get_provider_type()
                      
    if provider_type == 'aws' and request.method == 'GET':
        return redirect(url_for('auth.saml_aws', sp_id=sp_id))
    elif provider_type == 'jenkins' and request.method == 'GET':
        return redirect(url_for('auth.saml_jenkins', sp_id=sp_id))
    elif provider_type == 'github' and request.method == 'GET':
        return redirect(url_for('auth.saml_github', sp_id=sp_id))
...
def saml_aws(sp_id):
    ...
    sp = ServiceProvider.objects(id=sp_id).first()
    if sp:
        handler = provider_registry.get_handler_by_type('aws')
        return saml_manager.handle_provider_login(sp_id, handler)
    return redirect(url_for('main.dashboard'))
...
def saml_jenkins(sp_id):
    ...
    sp = ServiceProvider.objects(id=sp_id).first()
    if sp:
        handler = provider_registry.get_handler_by_type('jenkins')
        return saml_manager.handle_provider_login(sp_id, handler, relay_state)
    return redirect(url_for('main.dashboard'))

@auth_bp.route('/saml/github/<sp_id>', methods=['GET'])
@login_required
def saml_github(sp_id):
    """Generate and POST SAML Response for GitHub"""
    relay_state = request.args.get('RelayState')
    sp = ServiceProvider.objects(id=sp_id).first()
    if sp:
        handler = provider_registry.get_handler_by_type('github')
        return saml_manager.handle_provider_login(sp_id, handler, relay_state)
    return redirect(url_for('main.dashboard'))
